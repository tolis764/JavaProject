...
//                }
//            }
//            throw new JDocsException("Unable to store clock in information", e);
//
//        } finally {
//            if (session != null) {
//                closeSession(session);
...
//                }
//            }
//            throw new JDocsException("Unable to store clock out information", e);
//
//        } finally {
//            if (session != null) {
//                closeSession(session);
...
//    }
    ...
    private boolean validate(User user, boolean editMode) throws JDocsException {
        ...
        if (query.list().size() > 0) {
            throw new JDocsException("Another user with same ID already exists");
        }
    ...
    public void saveOrUpdate(User user, boolean editMode) {
        ...
        try {
            ...
            throw new JDocsException("Could not save user", x);
        } finally {
            closeSession(session);
        }
    ...
    // }
    // }
    public int findNumberOfOpenTickets(User user) throws JDocsException {
        Session session = null;
        Transaction tx = null;
...
//			tx.commit();
            ...
            throw new JDocsException("Unnable to find user", e);
        } finally {
            if (session != null) {
                session.close();
            }
